...
SculptView3D lets you drag and drop a 3D model file directly into your browser and inspect it with full rotation, panning, scaling, and lighting controls — no installation, no account, no data sent anywhere. It supports standard and side-by-side stereo 3D viewing modes.
...
> **Tip:** For textured OBJ models, select and drop the `.obj`, `.mtl`, and all texture image files at the same time. The viewer resolves them automatically.

---

## Getting Started

1. Open `SculptView3D_DrewMedina.html` in a modern web browser (Chrome, Firefox, Edge, or Safari).
...
Open with the **⚙ Settings** button. Contains:
...
- **Display** — wireframe toggle, grid toggle, shading mode (Textured / Flat / MatCap), model scale slider
- **Rotation Axis Lock** — X, Y, Z toggle buttons to prevent rotation on individual axes; **⬆ Set View as Rotation Origin** resets the rotation reference to the current view so Y-drag spins correctly
...
Your 3D model files are never uploaded to any server. All file loading, parsing, and rendering happens entirely inside your browser using the [File API](https://developer.mozilla.org/en-US/docs/Web/API/File_API) and [WebGL](https://developer.mozilla.org/en-US/docs/Web/API/WebGL_API). Close the tab and the files are gone from memory.
...
These are well-maintained, widely audited open-source libraries. If you need to run the viewer in a fully offline or air-gapped environment, download these files locally and update the `<script type="importmap">` block to point to your local copies.
